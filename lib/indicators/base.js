...
Base.prototype.getDataSegment = function(length) {
    ...
    var dataSegmentLength = Math.min(length, this.tickCount);

    return data.slice(this.tickCount - dataSegmentLength, this.tickCount);
};

Base.prototype.getPrevious = function() {
    var data = this.getData();
    return data[this.tickCount - 2];
};

Base.prototype.getLast = function() {
    var data = this.getData();
    return data[this.tickCount - 1];
};
