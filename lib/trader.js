...
Trader.prototype.getStartingBalance = function() {
...
Trader.prototype.tick = function(tick) {
    ...
    var dayTradingBuyingPower = self.balance * 4;
    ...
    if (signal === TradeSignaler.BUY && !self.position) {
        ...
        self.position = {
            ...
            quantity: Math.floor((dayTradingBuyingPower - self.commission) / tick.askPrice),
            costBasis: (tick.askPrice * Math.floor((dayTradingBuyingPower - self.commission) / tick.askPrice)) + self.commission
        };
