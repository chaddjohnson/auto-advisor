...
TradeSignaler.prototype.tick = function(tick) {
    ...
    stopLossReached = ema <= self.averagePrice * (1 - (self.phenotype.stopLossThreshold / 100));
    ...
    if (!self.isTrading && !isTooLateToTrade && self.recentEmaChangeNegativeCount >= self.phenotype.emaChangeNegativeBuyThreshold && self.emaChangePositiveCount >= self.phenotype.emaChangePositiveBuyThreshold) {
        self.targetSellPrice = tick.askPrice * (1 + self.phenotype.targetIncrease);
        self.averagePrice = tick.askPrice;
        justSignaled = true;
        signal = TradeSignaler.BUY;
    }

    // Sell if didn't just buy and tick bid price exceeds target price.
    if (self.isTrading && !justSignaled && ((self.targetSellPrice && tick.bidPrice >= self.targetSellPrice) || stopLossReached || isDayEnd)) {
